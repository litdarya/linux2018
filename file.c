...
int writeBlock(char* bytes, FILE* save_file) {
    char buffer[block_size];
    memset(buffer, 0, block_size);
    memcpy(buffer, bytes, strlen(bytes));
    return fwrite(buffer, 1, block_size, save_file);
}
...
char* GetFromFile(int begin, int end) {
    ...
    fread(res, 1, end - begin, save_file);
    fclose(save_file);
    res[end - begin] = '\0';
    return res;
}
...
char* ReadBlock(int* pos) {
    ...
    fread(res, 1, block_size, save_file);
    fclose(save_file);
    res[block_size] = '\0';
    return res;
}
...
//        printf("Error opening file!\n");
    ...
    fseek(save_file, *pos, SEEK_SET);
    writeBlock(block, save_file);
//    fwrite(block, sizeof(char), strlen(block), save_file);
    fclose(save_file);
...
//    return Write(dir_name);
    if(strlen(dir_name) > block_size) {
        char msg[] = "Error! File name should be less \n";
        send(sock, msg, sizeof(msg), 0);
